...
def test_password_change_flow():
    ...
    try:
        ...
        if not Profile.objects.filter(user=user).exists():
            ...
            profile.save()
        print(f"✅ Validador creado: {user.username}")
    except Exception as e:
        print(f"❌ Error al crear validador: {e}")
        import traceback
        traceback.print_exc()
        return False
    
    # Intentar acceder a una página protegida (debería redirigir a change_password)
    print("\n📝 Paso 1: Iniciar sesión como validador con TempPass123!")
    client.login(username='test_validator', password='TempPass123!')
    print("✅ Sesión iniciada")
    
    # Intentar acceder a /validator1/
    print("\n📝 Paso 2: Intentar acceder a /validator1/ (debería redirigir a /change-password/)")
    ...
    if response.status_code == 302:
        ...
        if 'change-password' in location:
            print("✅ Correctamente redirigido a cambio de contraseña")
        else:
            print(f"❌ Redirección incorrecta: {location}")
            return False
    else:
        print(f"❌ No hubo redirección (status: {response.status_code})")
        return False
    
    # Acceder a la página de cambio de contraseña
    print("\n📝 Paso 3: Acceder a /change-password/")
    response = client.get('/change-password/')
    print(f"   Status code: {response.status_code}")
    if response.status_code == 200:
        print("✅ Página de cambio de contraseña accesible")
        if 'Cambiar Contraseña' in response.content.decode():
            print("✅ Formulario de cambio de contraseña presente")
        else:
            print("❌ Formulario no encontrado")
            return False
    else:
        print(f"❌ No se pudo acceder (status: {response.status_code})")
        return False
    ...
    print(f"   Status code: {response.status_code}")
    if response.status_code == 302:
        print("✅ Contraseña cambiada (redirección 302)")
        ...
        if 'validator1' in location or 'dashboard' in location or 'home' in location:
            print("✅ Redirección al panel correcta")
        else:
            print(f"⚠️  Redirección inesperada: {location}")
    else:
        print(f"❌ Status inesperado: {response.status_code}")
        return False
    
    # Verificar que ya no redirige a cambio de contraseña
    print("\n📝 Paso 5: Verificar que ya no redirige a cambio de contraseña")
    response = client.get('/validator1/', follow=False)
    print(f"   Status code: {response.status_code}")
    if response.status_code == 200 or (response.status_code == 302 and 'change-password' not in response.get('Location', '')):
        print("✅ Ya no redirige a cambio de contraseña")
    else:
        print(f"❌ Aún redirige a cambio de contraseña")
        return False
    
    # Limpiar
    user.delete()
    print("\n✅ TEST EXITOSO: Cambio obligatorio de contraseña funciona correctamente\n")
    return True
...
def test_validator_redirect():
    ...
    try:
        ...
        if not Profile.objects.filter(user=user_v1).exists():
            ...
            profile_v1.save()
        print(f"✅ Validador nivel 1 creado: {user_v1.username}")
    except Exception as e:
        print(f"❌ Error: {e}")
        import traceback
        traceback.print_exc()
    ...
    try:
        ...
        if not Profile.objects.filter(user=user_v2).exists():
            ...
            profile_v2.save()
        print(f"✅ Validador nivel 2 creado: {user_v2.username}")
    except Exception as e:
        print(f"❌ Error: {e}")
        import traceback
        traceback.print_exc()
    ...
    try:
        ...
        if not Profile.objects.filter(user=user_admin).exists():
            ...
            profile_admin.save()
        print(f"✅ Admin creado: {user_admin.username}")
    except Exception as e:
        print(f"❌ Error: {e}")
        import traceback
        traceback.print_exc()
    ...
    print("\n📝 Paso 1: Validador 1 accede a /home/ (debería redirigir a /validator1/)")
    ...
    if response.status_code == 302:
        ...
        if 'validator1' in location:
            print("✅ Validador 1 redirigido correctamente")
        else:
            print(f"❌ Redirección incorrecta: {location}")
            user_v1.delete()
            user_v2.delete()
    ...
    print("\n📝 Paso 2: Validador 2 accede a /home/ (debería redirigir a /validator2/)")
    client.logout()
    ...
    if response.status_code == 302:
        ...
        if 'validator2' in location:
            print("✅ Validador 2 redirigido correctamente")
        else:
            print(f"❌ Redirección incorrecta: {location}")
            user_v1.delete()
            user_v2.delete()
    ...
    # Test admin redirect
    print("\n📝 Paso 3: Admin accede a /home/ (debería redirigir a /dashboard-admin/)")
    client.logout()
    ...
    user_v2.delete()
    user_admin.delete()
    print("\n✅ TEST EXITOSO: Redirecciones funciona correctamente\n")
    return True
...
def test_upload_form_hidden():
    ...
    try:
        ...
        if not Profile.objects.filter(user=user_client).exists():
            ...
            profile_client.save()
        print(f"✅ Cliente creado: {user_client.username}")
    except Exception as e:
        print(f"❌ Error: {e}")
        import traceback
        traceback.print_exc()
    ...
    try:
        ...
        if not Profile.objects.filter(user=user_val).exists():
            ...
            profile_val.save()
        print(f"✅ Validador creado: {user_val.username}")
    except Exception as e:
        print(f"❌ Error: {e}")
        import traceback
        traceback.print_exc()
    ...
    print("\n📝 Paso 1: Cliente accede a /home/ (debería ver formulario)")
    ...
    if response.status_code == 200:
        ...
        if 'Subir documento' in content and 'enctype="multipart/form-data"' in content:
            print("✅ Formulario visible para cliente")
        else:
            print("❌ Formulario no encontrado para cliente")
            user_client.delete()
            user_val.delete()
            return False
    else:
    ...
    print("\n📝 Paso 2: Validador accede a /home/ (será redirigido)")
    client.logout()
    client.login(username='test_val_hide', password='ValPass123!')
    response = client.get('/', follow=False)
    if response.status_code == 302:
        print(f"✅ Validador redirigido (no ve formulario)")
    else:
        print(f"⚠️  Validador no fue redirigido (status: {response.status_code})")
    
    # Limpiar
    user_client.delete()
    user_val.delete()
    print("\n✅ TEST EXITOSO: Ocultación del formulario funciona\n")
    return True
